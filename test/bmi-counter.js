...
test('bmi state', function t(assert) {
    bmiCounter.state.first().subscribe(function onNext(state) {
...
test('update weight', function t(assert) {
    ...
    raf(function afterRender() {
        var state = comp.proxy.state;
        assert.equal(state.weight, 120);
...
test('update height', function t(assert) {
    ...
    raf(function afterRender() {
        assert.equal(comp.proxy.state.height, 210);
        assert.equal(comp.proxy.state.weight, 80);
...
test('update bmi', function t(assert) {
    ...
    raf(function afterRender() {
        assert.equal(comp.proxy.state.height, 180);
        assert.equal(comp.proxy.state.weight, 87.48);
